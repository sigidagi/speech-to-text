...
func Process(model whisper.Model, path string, flags *Flags) (string, error) {
	var data []float32
	...
	if buf, err := dec.FullPCMBuffer(); err != nil {
		...
		return str, fmt.Errorf("unsupported sample rate: %d", dec.SampleRate)
	} else if dec.NumChans != 1 {
		return str, fmt.Errorf("unsupported number of channels: %d", dec.NumChans)
	} else {
	...
	str, err = OutputToString(context)
	if err != nil {
		return str, err
	}
...
func OutputToString(context whisper.Context) (string, error) {
